...
export function useFirestoreDocument<T extends { id?: string }>(
  ...
  ) => firebase.firestore.DocumentReference<T> | Falsy,
  deps: DependencyList,
  ...
  useEffect(() => {
    ...
    if (resolved && resolved.onSnapshot) {
      const descriptor = (resolved as any).path;

      debug("Loading " + descriptor);

      function snapshotHandler(snapshot: firebase.firestore.DocumentSnapshot) {
