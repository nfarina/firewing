...
export function parseSql(query, parseCond) {
  ...
  var keywords = [
    ...
    // "HAVING",
    "WHERE",
    // "LIMIT", // NICK: causes problems when column names include "limit"
    // "VALUES",
    // "SET",
  ];
...
CondLexer.prototype = {
  ...
  readWord: function () {
    ...
    if (/^(IN|IS|NOT|LIKE|HAS)$/i.test(tokenValue))
